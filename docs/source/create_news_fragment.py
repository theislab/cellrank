...
def _parse_description(comment: str) -> str:
    def generate(lines: List[str]) -> str:
        ...
        if all(not len(line) for line in lines):
            raise ValueError(
                f"All description lines are empty after removing leading/trailing whitespace."
            )
...
def create_news_fragment(
    ...
    try:
        ...
        logging.info(
            f"Generating `{typ}` news fragment generation for issue `{issue_number}`"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Automatically create news fragment from an issue number."
    )
    parser.add_argument(
        "issue_number",
        type=str,
