...
class TestRandomWalk:
    def test_matrix_not_row_stochastic(self, test_matrix_4: np.ndarray):
        with pytest.raises(
            ...
            _ = RandomWalk(None, test_matrix_4)
    ...
    def test_reproducibility(self, test_matrix_1: np.ndarray, n_sims: int):
        rw1 = RandomWalk(None, test_matrix_1)
        rw2 = RandomWalk(None, test_matrix_1)
    ...
    def test_simulate_one(self, test_matrix_1: np.ndarray):
        res = RandomWalk(None, test_matrix_1).simulate_one(max_iter=10)
    ...
    def test_start_ixs(self, test_matrix_1: np.ndarray):
        res = RandomWalk(None, test_matrix_1, start_ixs=[1]).simulate_one(max_iter=10)

        assert res[0] == 1

    def test_stop_ixs(self, test_matrix_1: np.ndarray):
        res = RandomWalk(None, test_matrix_1, stop_ixs=[1]).simulate_one(
    ...
    def test_successive_hits(self, test_matrix_1: np.ndarray):
        ...
        res = RandomWalk(None, test_matrix_1, stop_ixs=[0]).simulate_one(
    ...
    def test_simulate_many(self, test_matrix_1: np.ndarray, backend: str, n_jobs: int):
        res = RandomWalk(None, test_matrix_1).simulate_many(
