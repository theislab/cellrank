...
class TestModel:
    ...
    def test_prepare_invalid_gene(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        with pytest.raises(KeyError):
            model.prepare("foo", "0")

    def test_prepare_invalid_lineage(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        with pytest.raises(KeyError):
            model.prepare(adata_cr.var_names[0], "foo")

    def test_prepare_invalid_data_key(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        with pytest.raises(KeyError):
            model.prepare(adata_cr.var_names[0], "0", data_key="foo")

    def test_prepare_invalid_time_key(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        with pytest.raises(KeyError):
            model.prepare(adata_cr.var_names[0], "0", time_key="foo")
    ...
    def test_prepare_normal_run(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        model = model.prepare(adata_cr.var_names[0], "0")
    ...
    def test_prepare_n_test_points(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        model = model.prepare(adata_cr.var_names[0], "0", n_test_points=300)

        assert len(model.x_test) == 300

    def test_predict(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        model = model.prepare(adata_cr.var_names[0], "0").fit()
        y_hat = model.predict()
    ...
    def test_confidence_interval(self, adata_cr: AnnData):
        model = create_model(adata_cr)
        model = model.prepare(adata_cr.var_names[0], "0").fit()
        _ = model.predict()
