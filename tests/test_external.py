...
class TestMoscotKernel:
    ...
    def test_compute_transition_matrix_from_output(
        ...
        tmaps = {p.solution for p in moscot_tp.problems}
    ...
    def test_invalid_solver_kwargs(self, adata_large: AnnData):
        mk = MoscotKernel(adata_large, time_key="age(days)")
        with pytest.raises(TypeError, match="unexpected keyword argument 'foo'"):
            mk.compute_transition_matrix(foo="bar")

    def test_inversion_updates_adata(self, adata_large: AnnData):
        key = "age(days)"
        mk = MoscotKernel(adata_large, time_key=key)
        assert is_categorical_dtype(adata_large.obs[key])
        assert adata_large.obs[key].cat.ordered
        np.testing.assert_array_equal(mk.experimental_time, adata_large.obs[key])
    ...
    def test_update_colors(self, adata_large: AnnData, cmap: str):
        ckey = "age(days)_colors"
        _ = MoscotKernel(adata_large, time_key="age(days)", cmap=cmap)
    ...
    def test_self_transitions(self, moscot_tp: TemporalProblem, st: SelfTransitions):
        ...
        mk = MoscotKernel.load(moscot_tp, time_key=key).compute_transition_matrix(
            self_transitions=st,
            conn_weight=0.2,
            conn_kwargs=conn_kwargs,
            threshold=None,
