...
class DisplacementKernel(ConnectivityMixin, BidirectionalKernel):
    ...
    def _read_from_adata(
        ...
        **kwargs: Any,
        ...
        self._xdata = self._extract_layer(xkey)
        self._vdata = self._extract_layer(vkey)
        self._vexp = get_moments(self.adata, self._vdata, second_order=False).astype(
            np.float64, copy=False
        )
        self._vvar = get_moments(self.adata, self._vdata, second_order=True).astype(
            np.float64, copy=False
        )
    ...
    def compute_transition_matrix(
        ...
        **kwargs: Any,
        ...
        # fmt: off
        params = {"softmax_scale": softmax_scale, "model": model, "similarity": str(similarity)}
        if self.backward:
