...
#     2. :meth:`cellrank.tl.estimators.GPCCA.set_terminal_states_from_metastable_states`
...
# can be useful when the terminal states are known beforehand. In this case, we don't need to compute the metastable
# states.
#
...
# %%
# Set terminal states from metastable states
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# :meth:`cellrank.tl.estimators.GPCCA.set_terminal_states_from_metastable_states` sets the terminal states by subsetting
