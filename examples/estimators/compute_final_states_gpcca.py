...
# %%
# First, we need to compute the Schur vectors and metastable states. We refer the reader to
...
#
# We will cover each of these methods below. In the last 2 cases, parameter ``n_cells`` controls how many of the most
# probable cells from each final state we take as a categorical annotation. Towards these we can later compute the
...
# states.
#
# The states can be specified either as a categorical :class:`pandas.Series` where `NaN` marks a cell not belonging to
# any final state or a :class:`dict`, where keys correspond the names of the final states,
...
# one, as shown below, where we combine `'Alpha'` and `'Beta'` states into a new one.
...
# we can plot the final states in one plot or separately (``same_plot=...``) or whether we plot the probabilities
